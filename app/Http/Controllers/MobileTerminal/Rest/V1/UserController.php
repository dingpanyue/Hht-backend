...
class UserController extends BaseController
{
    ...
    public function offlineMessages()
    {
        ...
        $userModel = new User();
        $users = $userModel->select('users.id', 'users.name', 'users.image')->leftJoin('messages', 'to_user_id','users.id')->where('to_user_id', $user->id)->where('messages.status', Message::STATUS_UNSENT)->groupBy('users.id')->get();
