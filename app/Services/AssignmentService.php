...
class AssignmentService
{
    ...
    {
        $assignment = $this->assignmentEloqument->with('user')->with('acceptedAssignments')->with('acceptedAssignments.serveUser')->with('acceptedAssignments.serveUser.userInfo')->with('user.userInfo')->find($assignmentId);
        return $assignment;
    }
    ...
    {
        //自己发布的委托
        $assignments = $this->assignmentEloqument->with('acceptedAssignments')->with('adaptedAssignment')->where('user_id', $user->id)->orderBy('status', 'desc')->orderBy('updated_at', 'asc');
